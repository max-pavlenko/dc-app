...
import {updateFriendInvitation} from "../socket.io/events/friend-invitation";
...
export async function sendFriendInvite(req: Request, res: Response) {
    ...
    console.log(friend, friendEmail, 'friend');
    ...
    if (areUsersFriends) return res.status(409).send({error: 'You are already a friend with this user. Are you best friends?'});
